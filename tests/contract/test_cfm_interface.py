...
def populated_cfm() -> CanonicalFunctionalModel:
    ev = EvidenceRef(
        graph_node_id="n0", document_id="doc1", section_id="s1", text="src"
    )
    return CanonicalFunctionalModel(
        run_id="test",
        actors={"a1": Actor(id="a1", name="Admin", evidence=ev)},
        functional_processes={
            "fp1": FunctionalProcess(id="fp1", name="Login", evidence=ev)
        },
        business_rules={"b1": BusinessRule(id="b1", name="Rule1", evidence=ev)},
        data_groups={"d1": DataGroup(id="d1", name="Data1", evidence=ev)},
        relationships=[
            Relationship(id="r1", source_id="a1", target_id="d1", evidence=ev)
        ],
        operations={
            "o1": Operation(id="o1", name="Op1", parent_process_id="fp1", evidence=ev)
        },
...
class TestCfmContract:
    def test_all_six_categories_accessible(
        self, populated_cfm: CanonicalFunctionalModel
    ) -> None:
    ...
    def test_no_framework_labels(self, populated_cfm: CanonicalFunctionalModel) -> None:
        names = []
        for collection in (
            populated_cfm.actors,
            populated_cfm.business_rules,
            populated_cfm.data_groups,
            populated_cfm.operations,
        ):
        ...
        assert not any(
            keyword in name for name in names for keyword in framework_keywords
        )
