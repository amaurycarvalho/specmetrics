...
class TestEvidenceGraphPipeline:
    def test_stage_builds_graph_from_extraction_result(self) -> None:
        ...
        elem = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref, content="a fact"
        )
    ...
    def test_stage_handles_multiple_providers(self) -> None:
        ...
        elem1 = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref1, content="fact 1"
        )
        elem2 = ExtractedElement(
            id="e2", type="entity", confidence=0.8, evidence=ref2, content="entity 1"
        )
        r1 = ExtractionResult(
            elements=[elem1],
            provider_id="p1",
    ...
    def test_update_for_document_replaces_nodes(self) -> None:
        ...
        elem = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref, content="old fact"
        )
        ...
        backend.add_node(
            eid, {"node_type": "evidence", "document_id": "doc1", "text": "evidence"}
        )
        backend.add_edge(nid, eid, {"edge_type": "derived_from"})
        backend.add_node(
            "other", {"node_type": "evidence", "document_id": "doc2", "text": "other"}
        )

        stage = EvidenceGraphStage(backend=backend)
        new_elem = ExtractedElement(
            id="e2", type="entity", confidence=0.7, evidence=ref, content="new entity"
        )
