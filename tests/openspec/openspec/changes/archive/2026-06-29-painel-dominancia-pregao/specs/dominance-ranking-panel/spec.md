...
Define the Dominance Ranking panel — a cross-sectional visualization in the Análise Geral tab that ranks all tickers by their CLV (Close Location Value) for the most recent trading session, answering "who won the session?" at a glance.
...
O sistema DEVE codificar visualmente dominância, intensidade e classificação qualitativa em cada barra, sem competição entre canais visuais.
...
### Requirement: Círculo de Money Flow Volume (DR103)

O sistema DEVE exibir um círculo na extremidade de cada barra cujo diâmetro representa o Money Flow Volume acumulado do período para aquele ticker.

#### Scenario: Círculo proporcional ao MFV

- **WHEN** um ticker tem MFV = R$ 10M e outro MFV = R$ 1M
- **THEN** o diâmetro do círculo do primeiro DEVE ser maior que o do segundo

#### Scenario: MFV zero ou negativo

- **WHEN** MFV = 0 ou não disponível
- **THEN** o círculo NÃO DEVE ser exibido para aquele ticker

### Requirement: Tooltip informativo (DR104)

O sistema DEVE exibir um tooltip ao passar o mouse sobre cada barra ou círculo, com detalhes do ticker.
