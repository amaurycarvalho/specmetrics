...
class TestNetworkXBackend:
    def test_add_node_stores_attributes(self, backend: NetworkXBackend) -> None:
        backend.add_node(
            "n1", {"node_type": "evidence", "document_id": "doc1", "text": "hello"}
        )
    ...
    def test_serialization_round_trip(self, backend: NetworkXBackend) -> None:
        ...
        backend.add_node(
            "n2", {"node_type": "extracted_element", "document_id": "doc1", "text": "b"}
        )
    ...
    def test_build_from_extraction_result_produces_correct_counts(self) -> None:
        ...
        ref = EvidenceReference(
            document_id="doc1", section_id="s1", text="evidence text"
        )
        elem1 = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref, content="fact 1"
        )
        elem2 = ExtractedElement(
            id="e2", type="entity", confidence=0.8, evidence=ref, content="entity 1"
        )
        result = ExtractionResult(
            elements=[elem1, elem2],
            provider_id="test_provider",
            processing_stats=ProcessingStats(
                documents_processed=1, elements_extracted=2, errors=0, duration_ms=10
            ),
        )
        ...
        for elem in result.elements:
            nid = fingerprint_node(
                elem.evidence.document_id,
                elem.evidence.section_id,
                elem.evidence.text,
                elem.type,
            )
            try:
...
class TestIncrementalUpdate:
    def test_update_replaces_nodes_from_document(self) -> None:
        backend = NetworkXBackend()
        backend.add_node(
            "n1", {"node_type": "evidence", "document_id": "doc1", "text": "old"}
        )
        backend.add_node(
            "n2", {"node_type": "evidence", "document_id": "doc2", "text": "keep"}
        )
        ...
        ref = EvidenceReference(
            document_id="doc1", section_id="s1", text="new evidence"
        )
        elem = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref, content="new fact"
        )
        result = ExtractionResult(
            elements=[elem],
            provider_id="t",
            processing_stats=ProcessingStats(
                documents_processed=1, elements_extracted=1, errors=0, duration_ms=5
            ),
        )
    ...
    def test_update_preserves_other_documents(self) -> None:
        backend = NetworkXBackend()
        backend.add_node(
            "n1", {"node_type": "evidence", "document_id": "doc1", "text": "remove"}
        )
        backend.add_node(
            "n2", {"node_type": "evidence", "document_id": "doc2", "text": "keep"}
        )
        ...
        elem = ExtractedElement(
            id="e1", type="fact", confidence=0.9, evidence=ref, content="new"
        )
        from specmetrics.kernel.extraction_provider import (
            ExtractionResult,
            ProcessingStats,
    ...
    def test_update_empty_removes_all_document_nodes(self) -> None:
        backend = NetworkXBackend()
        backend.add_node(
            "n1", {"node_type": "evidence", "document_id": "doc1", "text": "gone"}
        )
        backend.add_node(
            "n2", {"node_type": "evidence", "document_id": "doc2", "text": "stay"}
        )
        stage = EvidenceGraphStage(backend=backend)
