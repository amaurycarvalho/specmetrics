...
- **Decision**: Measurement results are Pydantic models with `APFMeasurementResult` as the top-level container, containing `MeasuredFunction` entries with full evidence trails
- **Rationale**: Pydantic v2 is the project-standard modeling library. Structured output enables F10 (Export Layer) and F11 (Publisher) to consume results without parsing knowledge of APF internals.

### CFM Entity Mapping to APF Function Types

- **Decision**: Map CFM entities to APF function types using the following heuristic:
...
- **Rationale**: The CFM defines data groups and operations with functional semantics. These are natural APF candidates. The CFM must provide enough metadata (persistence type, operation direction, DET/RET/FTR counts) for deterministic classification.
- **Alternatives considered**: Requiring explicit APF type declarations in the CFM (would couple CFM to APF-specific concerns, violating Canonical Representation)
...
| F10 (Export Layer) | Consumes `APFMeasurementResult` structured output |
| F11 (Publisher) | Consumes `APFMeasurementResult` structured output |
