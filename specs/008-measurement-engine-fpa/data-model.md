# Data Model: FPA Measurement Engine

## Overview

Data entities for the FPA (IFPUG Function Point Analysis) measurement plugin. These models represent the measurement output — the result of applying deterministic counting rules to the Canonical Functional Model (F06). They are consumed by Export Layer (F10) and Publisher (F11) plugins.

## Entity Definitions

### FPAMeasurementResult

Top-level container for a complete FPA function point measurement.
...
| `function_type` | `FunctionType` | FPA classification: `ILF`, `EIF`, `EI`, `EO`, `EQ` |
...
CFM entities are classified to FPA function types using these deterministic rules:

| CFM Element Type | CFM Attribute Condition | FPA Function Type |
|-----------------|------------------------|-------------------|
| `DataGroup` | `data_type == "internal"` | ILF (Internal Logical File) |
| `DataGroup` | `data_type == "external"` | EIF (External Interface File) |
| `DataGroup` | `data_type == "shared"` | ILF (treated as internal for FPA purposes) |
...
Once constructed, `FPAMeasurementResult` is immutable. This guarantees:
